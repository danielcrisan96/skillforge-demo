...
#### F1.4 — Chat cu model real ⬜ _faza curentă_

Ce mai rămâne din F1: `src/app/api/chat/route.ts`, apelul real către model, legarea stream-ului la interfața de chat și `docs/anthropic/README.md`.

Partea grea e deja făcută: clientul din F1.3 rămâne aproape neschimbat, pentru că protocolul e același. Se schimbă doar sursa bucăților — array-ul fix din `api/about/route.ts` devine răspunsul providerului.

Punctul de înlocuire în interfață e tot unul singur și e marcat în cod: acțiunea `sendMessage` din `src/store/useAppStore.ts`, care acum pune un text fix din `src/lib/mock/conversations.ts`.
